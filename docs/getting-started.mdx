...
2. **Typesense Lifecycle Automation**: Simplifies deploying, scaling, and managing Typesense clusters.
    ...
    It handles aspects as:
     ...
     - deploying Typesense as a `StatefulSet`, each `Pod` contains two containers:
     ...
     - provisioning Typesense services (headless & discovery `Services`),
     - actively discovering and updating Typesense's nodes list (quorum configuration mounted as `NodesListConfigMap`).
